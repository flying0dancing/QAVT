...
Option Explicit
Dim file,rowcount,fso,oexcel,openexcel,osheet,logfile,log_tmp,rh,sheetname,Forms_str,Forms_arr

If WScript.Arguments.length=3 Then
	file=Trim(WScript.Arguments(0))
	Forms_str=Trim(WScript.Arguments(1))
'	rowcount=WScript.Arguments(2)
	logfile=Trim(WScript.Arguments(2))
Else
